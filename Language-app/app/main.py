...
app.config['MAIL_USERNAME'] = os.environ.get("EMAIL_Language")
app.config['MAIL_PASSWORD'] = os.environ.get("PASSWORD_Language")
...
admin = Admin(app, name='Language Admin', template_mode='bootstrap4')
...
class RegisterForm(FlaskForm):
    ...
    password = PasswordField(validators=[
        InputRequired(), Length(min=6), ], render_kw={"placeholder": "Password (6 minimum)"})
...
class LangSearchForm(FlaskForm):
    lang = StringField(validators=[InputRequired(), Length(
        min=4, max=15)], render_kw={"placeholder": "Search For Languages"})
...
def login():
    ...
    if registerform.validate_on_submit():
        ...
        password_strength = password_check(registerform.password.data)
        if password_strength['password_ok']:
            ...
            flash("Password doesn't meet the security criteria: "+ str(password_strength))
...
def dashboard():
    ...
    if form.validate_on_submit():
        ...
        for p in total_posts:
            if str(form.lang.data).lower() in str(p.post_title).lower():
                search_lang.append(p)
            elif str(form.lang.data).lower() in str(p.post_content).lower():
                search_lang.append(p)
        if search_lang:
            flash("Search results:")
            return render_template("dashboard_search.html", posts=search_lang, title="My Dashboard", total_posts=len(search_lang), form=form)
            # redirect(url_for('user', username=form.lang.data))
        else:
            flash("Course of this language does not exist.")
...
def search_language(lang):
    user = Post.query.filter_by(post_title=lang).all()
    page = request.args.get('page', 1, type=int)
    posts = user.posts.paginate(page=page, per_page=3)
    followers = Follower.query.filter_by(followed=user).all()
    followers_total = 0
...
def send_reset_email(user):
    ...
    msg = Message('Forgot your password?',
                  sender='Languagewebsite@gmail.com',
